...
<!-- [ ] make a screenshot of elements to scrape -->
...
```{r}
#| label: ad-fed-papers-loc-url-ul-li-a-text
...
Assign the URLs to fed_urls and the titles to fed_titles

```{r}
#| label: ad-fed-papers-loc-url-ul-li-a-text-assign
...
Retrieve the HTML for each of the nine pages in `fed_urls` and assign to `fed_html`

```{r}
#| label: ad-fed-papers-read-html

fed_html <- fed_urls |> 
  map(read_html)
```

Write this HTML to files in the `../data/original/federalist_papers/` directory with the names from `fed_titles`

```{r}
#| label: ad-fed-papers-write-html
#| eval: false

fed_html |> 
  map2(fed_titles, ~write_html(.x, paste0("../data/original/federalist_papers/", .y, ".html")))
```

::: {.callout}
**{{< fa regular hand-point-up >}} Tip**

- Iteration: `message()`, `Sys.sleep()` 
- code to manage opening and closing web connections (`url()` and `close()`). 

Useful when we iterate over this function to provide status messages and to reduce server load when processing multiple webpages from a web domain. (3) will be necessary to manage webpages that are non-existent. 
:::

<!-- Web scraping: acquire raw HTML: process in curation step -->


<!-- Efficient code -->

This will work, but as we have discussed before one of this goals we have we acquiring data for a reproducible research project is to make sure that we are developing efficient code that will not burden site's server we are scraping from. In this case, we would like to check to see if the data is already downloaded. If not, then the script should run. If so, then the script does not run. Of course this is a perfect use of a conditional statement. 
