...
<!-- [ ] !: Remove links and make sure that these sources are in the Guide on the Resource Kit site -->
...
<!-- [ ] !: Pkg citations!! find first occurrence -->
...
Now, before we move on, we need to make sure to document the data. Use the the `create_data_origin()` function from the `qtalrkit` package to create a data origin file template in CSV format. The function only needs to be run once and does not need to be part of the reproducible workflow.
