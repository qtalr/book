...
@exm-ad-fed-papers-read-write-function might not seem like a step up from @exm-ad-fed-papers-read-write-html-manual, but it is. We can now use the `map() ` function from `purrr` to iterate over the URLs in `fed_urls` and apply the `read_write_html()` function to each URL. See @exm-ad-fed-papers-read-write-map.
