...
- Introduction
  - Collect and curate your references and resources with a reference manager (e.g., Zotero, Mendeley, EndNote, etc.)
    - Manage your ideas and keep notes
    - Organize your references and resources
    - Integrate your references and resources with your writing in Quarto through BibTeX entry citation keys

- Methods
  - Document your data and processing steps while conducting your research
    - Use a data dictionary to describe your data
    - Use literate programming to describe your data processing and analysis steps

- Results
  - Save key statistical results, tables, and visualizations as outputs
    - Statistical results can be saved as R objects (e.g., `saveRDS()`)
    - Tables and visualizations can be saved as files (e.g., `ggsave()`, `kableExtra::save_kable()`)

- Publishing
  - Presentations
    - Various presentation software (e.g., PowerPoint, Keynote, Google Slides, etc.) and slide sharing platforms (e.g., SlideShare, Speaker Deck, etc.)
    - Consider using Quarto with [`revealjs`](https://quarto.org/docs/presentations/revealjs/) for creating and sharing presentations
      - Facilitates the integration of data, tables, and figures between presentations and articles
      - Easily shareable and reproducible
  - Articles
    - Quarto provides ample support for creating and publishing articles
      - `.csl` files for formatting references
      - Quarto extensions for (some) [journal-specific formatting](https://quarto.org/docs/extensions/listing-journals.html)
      - PDF, HTML, and Word output formats
...
```{r}
#| label: fig-ggsave-test
#| fig-cap: A test figure saved using `ggsave()`

library(ggplot2)    # plotting functions
library(dplyr)      # load the pipe operator
library(qtkit)      # load the `save_*()` functions
...
p1 <-
  ...
  geom_point() +
  geom_smooth() +
  labs(title = "Test figure", x = "Weight", y = "Miles (per gallon)")

p1

write_gg(p1, target_dir = "~/Desktop/")
```

```{r}
#| label: tbl-kable-test
#| tbl-cap: A test table saved using `kableExtra::save_kable()`

library(knitr)
library(kableExtra)

my_mtcars <- mtcars[1:5, 1:5] # Create a subset of the mtcars dataset
...
write_kbl(t1, target_dir = "~/Desktop/")
```

```{r}
#| label: obj-data-test

write_obj(my_mtcars, target_dir = "~/Desktop/")
```
