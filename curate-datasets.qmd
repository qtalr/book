...
tribble(
  ...
  "doc_id", "Document ID", "integer", "Unique identifier for each line in the corpus.",
  "type", "Type", "character", "Indicates whether the line is from the source or target language.",
  "line_id", "Line ID", "integer", "Unique identifier for each line for each `type`.",
  "line", "Line", "character", "Contains the text of the line, and maintains the structure of the original data."
) |>
  tt(width = 1)
```
...
#| tbl-cap: "Idealized structure for the ENNTT Corpus dataset."
#| tbl-colwidths: [10, 17, 19, 54]
#| echo: false
...
tibble::tribble(
  ...
  "type", "Type", "character", "Indicates whether the text is native, non-native, or translated",
...
Looks like the `extract_dat_attrs()` function is ready for prime-time. Let's now apply it to all of the line nodes in the `ns_dat_lines` object using the `map_dfr()` function from the `purrr` package [@R-purrr] as in @exm-cd-enntt-extract-attributes-all.
...
An example of a data dictionary, a data dictionary for the `europarl_curated_tbl` dataset is shown in @tbl-cd-unstructured-data-dictionary-example.
