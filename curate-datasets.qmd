...
As an example case, let's look at the `cabnc_participants` data frame. We can use the `skim()` function from the `skimr` package [@R-skimr] to get a summary of the variables in the dataset. We can add the `yank()` function to look at variable types one at a time. We will start with the character variables, as seen in @exm-cd-cabnc-skim-character.
