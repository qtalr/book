...
<!-- [ ] Update directory structure and reference to the data -->
...
<!-- [ ] consider making this display explicit -->
<!-- [ ] also consider removing the namespace prefixing, and just show the loading the the library -->
...
<!-- 
- [ ] make clear reference to "idealized structure" table, for each of the sections (unstructured, structured, semi-structured)
-->
...
# [ ] look into `readtext()` as I think we can read both files at once, although we will need to add a column to identify the language
...
<!-- [ ] I need to standardize how I refer to packages `readtext` -->
...
We have two objects now, one corresponding to the 'Source' and the other the 'Target' parallel texts. 


- [ ] Note: joining is a part of the **transformation** stage. 

Let's now join these two datasets, one on top of the other --that is, by rows. We wil use the `bind_rows()` function for this.
...
- [ ] Note: this is tokenization, a **transformation** step.
...
- [ ] Note: Regular Expressions are covered in the Swirl lesson 'Pattern Matching', but I'm not sure if I should be working with this here. Hmmm. I have to consider what to do with this chapter in terms of skills development.
...
write_csv(
  x = europarle_sentences_id, # object to write
  file = "../data/derived/europarle/europarle_curated.csv"
...
**What**: [Pattern Matching and Manipulate Datasets](https://lin380.github.io/tadr/articles/recipe_7.html)\
