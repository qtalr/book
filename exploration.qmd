...
We can clean up the results a bit by removing the POS tags from the `x` and `y` variables, up our minimum PMI value, and create a network plot to visualize the results in @exm-eda-masc-verb-part-network.
...
<!-- [ ] Let's focus on Lemma-POS, and not consider the others -->
...
We can apply PCA to each of these feature matrices and assess how well each accounts for the variation in the data. We can then select the feature approach that accounts for the most variation in the data for clustering. The `prcomp()` function from base R can be used to perform PCA.

Let's apply PCA to each of our candidate feature matrices, as seen in @exm-eda-masc-dtms-pca.
...
Now we can skip ahead and set our $k$ to 4 clusters in the `kmeans()` function. We can then visualize the results of the clustering algorithm with the `fviz_cluster()` function from the `factoextra` package. This function takes the output of the `kmeans()` function as its first argument and the matrix of features as its second argument, as seen in @exm-eda-masc-pos-kmeans-fit.
...
Overall, we can see that the clusters are relatively hetrogeneous and overlap with each other. There are some clusters that are more distinct than others, but there is no clear separation between the clusters. This is a good indication that the clusters are not well defined. This is not necessarily a bad thing, but it is something to consider when interpreting the results.
...
After we have our features, we can then apply the vector space model to the MASC dataset. When VSM is applied to words, it is known as **word embedding**. To calculate word embeddings there are various algorithms that can be used (BERT, word2vec, GloVe, *etc.*) We will use the **word2vec** [@Mikolov2013b] algorithm. Word2vec is a neural network-based algorithm that learns word embeddings from a large corpus of text. In the word2vec algorithm, the researcher can choose to learn embeddings from a **Continuous Bag of Words** (CBOW) or a **Skip-gram model**. The CBOW model predicts a target word based on the context words. The Skip-gram model predicts the context words based on the target word. The CBOW model is faster to train and is better for frequent words. The Skip-gram model is slower to train and is better for infrequent words.
