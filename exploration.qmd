...
Aligned in purpose with descriptive approaches, unsupervised learning approaches to exploratory data analysis are used to identify patterns in the data from an algorithmic perspective. Common methods in text analysis include principle component analysis, clustering, topic modeling, and word embedding.
...
Let's load the `topicmodels` package. Then we will clean the lemmas removing stop words. 

```{r}
#| label: eda-masc-topic-modeling
...
Next, we will create a document-term matrix (DTM) of the lemmas. We will use the `cast_dtm()` function.

```{r}
# Create a dtm of lemmas
...
```{r}
#| label: eda-masc-topic-modeling-lda
...
lemma_topics <- 
  lemma_lda_model |> 
  tidy(matrix = "beta") |> # beta: distribution of words in each topic. Gamma: distribution of topics in each document.
  group_by(topic) |>
  slice_max(beta, n = 10) |>
  ungroup() |>
...
So, if I want to see the words associated with the topics, use the 'beta' matrix. For the topics associated with each document, use the 'gamma' matrix. 

View the top words in each topic in a bar plot. 

```{r}
#| label: fig-eda-masc-topic-modeling-lda
#| fig-width: 8

# View the topics in a bar plot
lemma_topics |> 
