...
\index{science}Science starts with a question, identifies and collects data, careful selected slices of the complex world, submits this data to analysis through clearly defined and reproducible procedures, and reports the results for others to evaluate. This process is repeated, modifying, and manipulating the procedures, asking new questions and positing new explanations, all in an effort to make inroads to bring the complex into tangible view.

In essence what science does is attempt to subvert our inherent limitations by drawing on carefully and purposefully collected \index{sampling}samples of observable experience and letting the analysis of these observations speak, even if it goes against our intuitions (those powerful but sometime spurious heuristics that our brains use to make sense of the world).
...
The data science toolbelt also underlies well-known public-facing language applications. From the language-capable \index{chat applications}chat applications, \index{plagiarism detection software}plagiarism detection software, \index{machine translation algorithms}machine translation algorithms, and \index{search engines}search engines, tangible results of quantitative approaches to language are becoming standard fixtures in our lives.
...
Either based on the tenets of \index{theoretical frameworks}theoretical frameworks and/ or the objects of study of particular fields, \index{methodological approaches}methodological approaches to language research vary. On the one hand, some language research commonly applies qualitative assessment of language structure and/ or use. \index{qualitative approaches}**Qualitative approaches** describe and account for characteristics, or "qualities", that can be observed, but not measured (*e.g.* \index{introspective methods}introspective methods, \index{ethnographic methods}ethnographic methods, *etc.*)
...
Text analysis is a multifacited research methodology. It can be used use facilitate the qualitative\index{qualitative approaches} exploration of smaller, human-digestable textual information, but is more often employed quantitatively\index{quantitative approaches} to bring to the surface patterns and relationships in large samples of textual data that would be otherwise difficult, if not impossible, to identify manually.
...
To ensure that the results of text analysis projects are replicable\index{reproducible research} and transparent, programming\index{programming skills} strategies and documentation play an integral role at each stage of the implementation of a research project. While there are a number of programming languages that can be used for text analysis, R\index{R} widely adopted in linguistics research and is the language of choice for this textbook. R, in combination with literate programming\index{literate programming} and other tools, provides a robust and reproducible workflow for text analysis projects.

### Applications

So what are some applications of text analysis? Most public facing applications stem from Computational Linguistic research, often known as **Natural Language Processing**\index{Natural Language Processing} (NLP) by practitioners. Whether it be using search engines, online translators, submitting your paper to plagiarism detection software, *etc.* many of the text analysis methods we will cover are at play.
