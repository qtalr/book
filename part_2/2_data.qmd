...
Given the sampling frame and the strata and balance for this corpus visualized in @fig-data-brown-distribution, can you determine what language population this corpus aims to represent? What types of research might this corpus support or not support?
:::

::: {#second}
```{r}
#| label: fig-data-brown-distribution
...
brown <- read_csv("data/data-brown.csv")
...
|:-------|:------------|:---------|
| Language repositories | Repositories that specialize in language data | [Language Data Consortium](https://www.ldc.upenn
.edu/), [TalkBank](http://talkbank.org/) |
| Data sharing platforms | Platforms that enable researchers to securely store, manage, and share data | [GitHub](https://
github.com), [Zenodo](https://zenodo.org), [Open Science Framework](https://osf.io/)  |
...
**Unstructured data** is data which does not have a machine-readable internal structure. This is the case for plain text files (*.txt*), which are simply a sequence of characters. For example, in @lst-masc-text we see plain text file from the the Manually Annotated Sub-Corpus of American English (MASC) [@Ide2008]:
...
In @lst-masc-df we see an example of an R data frame object which overlaps with the language in the plain text file in @lst-masc-text:
...
**Semi-structured data** falls between unstructured and structured data. This covers a wide range of file structuring approaches. For example, a otherwise plain text file with part-of-speech tags appended to each word is minimally structured, @lst-masc-pos.
...
Towards the more structured end of semi-structured data, many file formats including *.xml* and *.json* contain hierarchical data. For example, in @lst-masc-xml shows a snippet from a *.xml* file from the MASC corpus.\
...
[![](figures/data-tidy.drawio.png)]{fig-alt="A visualization of a table where the row are marked as 'observations' and the columns are marked as 'variables' with each cell containing a 'value' that is a particular attribute of a particular observation for the particular observation-feature pair."}
...
[![](figures/data-transformations.drawio.png)]{fig-alt="Visualization of a table labeld 'row-wise' along the rows and 'column-wise' along the columns to highlight transformation operations on a dataset showing the original dataset in grey and the addition of either columns or rows in darker grey."}
...
![Concatenating](figures/integration-concat-visual.drawio.png){#fig-concatenating fig-alt="Two tables with three rows and three columns. The columns are labeled 'A', 'B', and 'C'. There is a vertical line with arrow ends suggesting combining the two tables into one table."}
...
Data acquired from corpus resources should be accompanied by information about the **data origin**. @tbl-data-data-origin provides a list of the types of information that should be included in the data origin information.

::: {#tbl-data-data-origin tbl-colwidths="[25, 75]"}
...
::: {#tbl-data-data-dictionary tbl-colwidths="[25, 75]"}
