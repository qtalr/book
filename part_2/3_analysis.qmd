...
# Data dictionary for the BELC dataset
...
Type-token ratio (TTR)\index{type-token ratio (TTR)} is a standard metric for measuring lexical diversity, but it is not without its flaws. Most importantly, TTR is highly sensitive to the word length of the text. @Duran2004 discusses this limitation, and the limitations of other lexical diversity\index{lexical diversity} measures and proposes a new measure $D$ which shows a stronger correlation with language proficiency in their comparative studies.\index{second language acquisition}j
:::

In @tbl-analysis-belc-overview, each of the variables is an attribute or measure of the `essay_id` variable. `tokens` is the number of total words, `types` is the number of unique words, `ttr` is the ratio of unique words to total words. This is known as the Type-Token Ratio and it is a standard metric for measuring lexical diversity. Finally, the proportion of L2 words (English) to the total words (tokens) is provided in `prop_l2`.
...
The central tendency\index{central tendency} is a  measure which aims to summarize the data points in a variable as the most representative, middle, or most typical value. There are three common measures of central tendency: the mode, mean and median. Each differ in how they summarize the data points.

The **mode**\index{mode} is the value that appears most frequently in a set of values. If there are multiple values with the highest frequency\index{frequency}, then the variable is said to be multimodal. A versatile central tendency measure, the mode can be applied all levels of measurement. However, in practice is almost exclusively used to summarize categorical variables.

The most common central tendency measures for numeric variables are the mean and the median. The **mean**\index{mean} is a summary statistic calculated by summing all the values and dividing by the number of values. The **median**\index{median} is calculated by sorting all the values in the variable and then selecting the middle value.
...
belc_essay_tbl |>
  ...
  to_titlecase(blacklist = c("group")) |>
  kable()
```
...
It is important to recognize that outliers\index{outliers} contribute undue influence to overall measures of central tendency\index{central tendency} and dispersion.\index{dispersion} To appreciate this, let's consider another helpful visualization called a **boxplot**\index{boxplot}. A boxplot is a visual representation which aims to represent the central tendency, dispersion, and distribution of a numeric variable in one plot.
...
p1 <-
  ...
  geom_vline(aes(xintercept = quants[[2]]), linetype = "solid", linewidth = 0.85) + # median
...
The last bivariate combination is numeric-numeric\index{numeric variables}. To summarize this type of relationship a scatterplot is used. A **scatterplot**\index{scatterplot} is a visualization that plots each data point as a point in a two-dimensional space, with one numeric variable on the x-axis and the other numeric variable on the y-axis. Depending on the type of relationship you are trying to assess, you may want to add a trend line to the scatterplot. A trend line is a line that summarizes the overall trend in the relationship between the two numeric variables. To assess the extent to which the relationship is linear, a straight line is drawn which minimizes the distance between the line and the points.

In @fig-analysis-belc-scatter-plot, we see a scatterplot and a scatterplot with a trend line for the relationship between `ttr` and `types` in the BELC dataset. We see there is an apparent positive relationship between these two variables, which is consistent with the idea that as the number of types increases, the type-token ratio increases.\index{type-token ratio (TTR)} In other words, as the number of unique words increases, so does the lexical diversity of the text. Since we are evaluating a linear relationship, we are assessing the extent to which there is a **correlation**\index{correlation} between `ttr` and `types`. A correlation simply means that as the values of one variable change, the values of the other variable change in a consistent manner.

<!-- ttr + types scatterplot: points, points + trend line -->

```{r}
#| label: fig-analysis-belc-scatter-plot
#| fig-cap: "Scatterplot for the relationship between `ttr` and `types`"
