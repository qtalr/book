...
::: {layout="[55, -3, 45]" layout-valign="top" layout-align="left"}
...
#| fig-alt: "Example distribution of the term 'Ya'll' the Word Mapper project. The southern states of the United States show a higher frequency of the term."
#| out-width: '100%'
...
```{r}
#| label: tbl-aa-belc-central-tendency
...
belc_essay_tbl |>
  ...
  tibble() |>
  tt(width = 1, digits = 2, theme = "striped")
...
As the mode is the most frequent value, the `top_counts` measure in @tbl-aa-belc-central-tendency provides the most frequent value for the categorical variables. Mean and median appear but we notice that the mean and median are not the same for the numeric variables. Differences that appear between the mean and median will be of interest to us later in this chapter.
...
In @tbl-aa-belc-dispersion-1, the normalized entropy helps us understand the balance of the values across the levels of the categorical variables. In @tbl-aa-belc-dispersion-2, the standard deviation and IQR provide a sense of the spread of the values around the mean and median, respectively, for the numeric variables.
...
When assessing the distribution of categorical variables, we can use a frequency table or bar plot. **Frequency tables** display the frequency and/ or proportion each level in a categorical variable in a clear and concise manner. In @tbl-aa-belc-frequency-table we see the frequency table for the variable `sex` and `group`.

<!-- sex frequency table -->

```{r}
#| label: tbl-aa-belc-frequency-table
#| tbl-cap: "Frequency table for variables in the BELC dataset"
#| tbl-subcap:
#|  - "Sex"
#|  - "Time group"
#| layout-ncol: 2
#| layout-valign: top
#| tbl-colwidths: [33.3, 33.3, 33.3]

belc_essay_tbl |>
  tabyl(sex) |>
  tibble() |>
  select(sex, frequency = n, proportion = percent) |>
  tt(width = 1, theme = "striped")
...
The goal of descriptive statistics is to summarize the data in a way that is meaningful and interpretable. With this in mind, compare the frequency tables in [-@tbl-aa-belc-frequency-table] and bar plots in [-@fig-aa-belc-barplots]. Does one provide a more interpretable summary of the data? Why or why not? Are there any other ways you might communicate this distribution more effectively?
:::
...
Simulations of these distributions appear in @fig-aa-distributions.
...
: Appropriate summary types for different combinations of variable types {#tbl-aa-summary-types}{tbl-colwidths="[17, 27, 28, 28]" .striped}
...
In @tbl-aa-belc-contingency-tables we see contingency tables for the categorical variable `sex` and ordinal variable `group` in the BELC dataset. A contingency table may include only counts, as in @tbl-aa-belc-contingency-tables-1, or may include proportions or percentages in an effort to normalize the counts and make them more comparable, as in @tbl-aa-belc-contingency-tables-2.
...
: Overview of analysis types {#tbl-aa-analysis-types}{tbl-colwidths="[15, 19, 22, 22, 22]" .striped}
...
[![](figures/a-eda-variables.drawio.png)]{fig-alt="A table which has columns labeled as 'feat_1', 'feat_2', on so on. Above, these columns are labeled as 'features'. This figure aims to show that no feature has a special status in exploratory data analysis."}
...
::: {#tbl-eda-methods tbl-colwidths="[50, 50]" .striped}
...
::: {#tbl-pda-algorithms tbl-colwidths="[50, 50]" .striped}
