...
A glimpse at the directory structure of the project at this point is seen in @lst-acquire-cedel2-structure.

::: {#lst-acquire-cedel2-structure}
...
As we can see looking at the directory structure, in @lst-acquire-swda-download-location, the *swda.tar.zip* file has been added to the *data/original/* directory.

::: {#lst-acquire-swda-download-location}
...
The directory structure of *data/* in @lst-acquire-swda-unarchive-location now shows the *swda.tar.gz* file and the *swda* directory that contains the unarchived directories and files.

::: {#lst-acquire-swda-unarchive-location}
```bash
data/
├── analysis/
├── derived/
...
Our directory structure in @exm-acquire-swda-untar-temp is the same as in @lst-acquire-swda-unarchive-location, minus the *swda.tar.gz* file.
...
In this case, we will put the function in a separate file, *functions.R*, in the same directory as the other process files as in @lst-acquire-functions-r.

::: {#lst-acquire-functions-r}
...
After running the `get_talkbank_data()` function, we can see that the data has been acquired and written to the *data/original/cabnc/* directory in @lst-acquire-functions-output

::: {#lst-acquire-functions-output}
```bash
data/
├── analysis
├── derived
