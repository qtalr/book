...
The CEDEL2 corpus is a freely available resource, but to access the data you must first use a search interface to select the relevant characteristics of the data of interest. Following the search/ download link you can find a search interface that allows the user to select the subcorpus and filter the results by a set of attributes, seen in @fig-acquire-cedel2-search.


::: {#fig-acquire-cedel2-search}

![](figures/acquire-cedel2-search.png){fig-alt="A view of the CEDEL2 Corpus search and download interface. The interface includes a search form with fields for subcorpus, results, language, and medium. The results are displayed in a table with columns for the title, author, language, medium, and download. The download link is not active until the search criteria are met."}

Search and download interface for the CEDEL2 Corpus
:::
...
In @tbl-acquire-cedel2-do, I've created a data origin file for the CEDEL2 corpus.
...
A glimpse at the directory structure of the project at this point is seen in @def-acquire-cedel2-structure.
