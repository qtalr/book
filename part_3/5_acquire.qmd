...
The directory structure of *data/* in @exm-ad-swda-unarchive-location now shows the *swda.tar.gz* file and the *swda* directory that contains the unarchived directories and files.

::: {#exm-ad-swda-unarchive-location}
```bash
data/
├── analysis/
├── derived/
...
Our directory structure in @exm-ad-swda-untar-temp is the same as in @exm-ad-swda-unarchive-location, minus the *swda.tar.gz* file.
...
First, we need to install and load {TBDBr}, as in @exm-ad-load-pacman.
...
{TBDBr}  provides a set of common `get*()` functions for acquiring data from the TalkBank corpus resources. These include:
