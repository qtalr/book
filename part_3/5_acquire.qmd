...
![](figures/ad-cedel2-search.png){fig-alt="A view of the CEDEL2 Corpus search and download interface. The interface includes a search form with fields for subcorpus, results, language, and medium. The results are displayed in a table with columns for the title, author, language, medium, and download. The download link is not active until the search criteria are met."}
...
![](figures/ad-cedel2-results.png){fig-alt="A view of the CEDEL2 Corpus search results. The results are displayed in a table with columns for the title, author, language, medium, and download. The download link is active for the search criteria 'L1 English - L2 Spanish'."}
...
# Write to disk
dput(utterances, file = "data/ad-utterances")
```

```{r}
#| label: ad-glimpse-utterances
#| echo: false

# Read from disk
utterances <- dget("data/ad-utterances")
