...
<!--
[ ] remove eval: false and store results to avoid depending on TBDBr
-->
```{r}
#| eval: false
...
Inspecting the output from @exm-ad-get-utterances, we see that the data frame contains `format(nrow(utterances), big.mark=",")` observations and `ncol(utterances)` variables.

<!-- [ ] add the 'r ...' back to the inline expressions -->
