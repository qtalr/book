...
: Workflow for inferential data analysis {#tbl-ida-workflow tbl-colwidths="[5, 15, 80]" .striped}
...
The `infer` package [@R-infer] provides a Tidyverse-friendly framework to implement simulation-based methods for statistical inference. Designed to be used in conjunction with the `tidyverse`, `infer` provides a set of functions that can be used to specify the variables of interest, calculate the observed statistic, generate the null hypothesis distribution and calculate the $p$-value and the confidence interval.
...
: Statistical test designs for categorical response variables {#tbl-ida-cat-design tbl-colwidths="[20, 35, 35, 10]" .striped}

We will use a derived version of the `dative` dataset from the `languageR` package. It contains over 3k observations describing the realization of the recipient clause in English dative constructions. To familiarize ourselves with the dataset, let's consider the data dictionary in @tbl-ida-cat-data-dict.
...
For a bit more context, a dative is the phrase which reflects the entity that takes the recipient role in a ditransitive clause. In English, the recipient (dative) can be realized as either a prepositional phrase (PP) as seen in @exm-ida-cat-dative-examples (1) or as a noun phrase (NP) as seen in (2).
...
To appreciate the null hypothesis distribution, we can visualize it using a histogram. The `infer` package provides a convenient `visualize()` function for visualizing distributions, as seen in @exm-ida-cat-null-hypothesis-vis.

::: {#exm-ida-cat-null-hypothesis-vis}
```r
...
```{r}
#| label: ida-cat-null-hypothesis-range
#| include: false
...
In @fig-ida-cat-null-hypothesis, we see that on the x-axis is the proportion statistic of NP realizations of the recipient clause that we would expect if the null hypothesis were true. For the 1,000 samples, the proportion statistic ranges from `r null_range[1]` to `r null_range[2]`. Importantly we can appreciate that the most of the proportion statistics are around 0.5, in fact, the mean is `r null_mean` with a standard deviation of `r null_sd`, which is what we would expect if the null hypothesis were true. But there is variation, as we would also expect.
...
We can cross-tabulate or visualize, but let's cross-tabulate this relationship as it is a basic 2-by-2 contingency table. In @exm-ida-cat-bivariate-tbl, we see the code for the cross-tabulation of the `realization_of_rcp` and `modality` variables. Note I've made use of the `janitor` package to adorn this table with percentages, totals, and observation numbers.
...
```{r}
#| label: ida-cat-null-hypothesis-bivariate
...
```{r}
#| label: ida-cat-confidence-interval-bivariate
...
#| fig-cap: "Distribution the variables `modality` and `length_ratio_log` by the levels of the `realization_of_rcp` variable."
#| fig-subcap:
#|  - "Realization of recipient clause by modality"
#|  - "Realization of recipient clause by length ratio"
#| layout-ncol: 2
...
```{r}
#| label: ida-cat-null-hypothesis-logistic-regression
...
```{r}
#| label: ida-cat-confidence-interval-logistic-regression
...
: Statistical test design for numeric response variables {#tbl-ida-num-design tbl-colwidths="[20, 35, 35, 10]" .striped}
...
fillers_tbl <-
  ...
  select(-speaker_id, -total_fillers, -total_utterances)
```
...
```{r}
#| label: fig-ida-num-uni-hist-dens-log
...
```{r}
#| label: ida-num-fillers-dataset-clean
#| echo: false

# Drop the jittered and non-log transformed variables
fillers_tbl <-
  fillers_tbl |>
  select(-fillers_per_100_jitter, -fillers_per_100)
```
...
#| fig-cap: "Boxplot of the `fillers_per_100_log` variable by `sex`"
...
```{r}
#| label: ida-num-fillers-type-dataset
