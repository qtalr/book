...
Let's first explore the distribution from a descriptive point of view. With a numeric response variable `fillers_orf_log` and a categorical explanatory variable `sex`, a box plot is a natural fit, as seen in @exm-infer-num-bi-vis.
...
A plot will help us begin to understand the potential relationships. In @exm-infer-multi-sex-plot, we use a box plot to visualize the relationship between the `fillers_orf_log` variable and the `filler_type` variable, with a `sex` overlay.
