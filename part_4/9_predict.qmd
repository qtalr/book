...
After tuning the `max_tokens` hyperparameter\index{hyperparameters}, the best performing value is 1,050. We now used the updated `cls_wf_lasso_tokens` workflow to cross-validate\index{k-fold cross fold validation} the model and collect the metrics. The code is seen in @exm-predict-class-tune-hyperparameters-tokenfilter-evaluate-workflow-cv-collect.
