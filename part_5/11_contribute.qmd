...
The research abstract is another important component of both presentations and articles. I have not included it in @tbl-ao-common-components as it is a distillation of the entire research narrative. Furthermore, the purpose of the abstract is distinct between presentations and articles. In presentations, the abstract typically serves to provide conference referees and attendees with a summary of the research. In articles, the abstract helps readers decide whether the article is relevant to their interests and worth the time to read in full.
...
```{r}
#| label: fig-contr-ro-structure
#| fig-cap: "Layers of a computational research environment"
#| out.width: "35%"
#| echo: false

include_graphics("figures/nested-project.drawio.png")
```

The research compendium is the most visible layer, as it is the primary means of interacting with the research project. The software layer includes R, R packages, and system-level dependencies. System-level dependencies serve to support the software layer. Software itself, these dependencies are not directly interacted with, but they are necessary for the more 'visible' software to function. Most people are familiar with operating systems, such as Windows, macOS, and perhaps Linux, but there are many different versions of these operating systems. Furthermore, hardware resources can vary widely. One of the most important aspects of hardware to consider for reproducibility is the architecture of the processor (the CPU).

For a quick overview of some of the elements of these layers, we can use `session_info()` from the `sessioninfo` package [@R-sessioninfo], as seen in @exm-contr-ro-session-info.

::: {#exm-contr-ro-session-info}
```{r}
#| label: contr-ro-session-info
...
```{r}
#| label: contr-ro-session-info-output
#| include: false

r <- session$platform$version
os <- session$platform$os
sys <- session$platform$system
```
:::

From the output of @exm-contr-ro-session-info, we can see that the platform information for the current session includes the `r r`, the operating system (`r os`), and the system (`r sys`).
...
A key component to research compendiums which integrate into a reproducible workflow is the use of a project structure that modularizes the research project into predictable and consistent components. This will will primarily consist of input, output, and the code that excutes the processing steps. But it will also consist of a coordinating script, that is used to orchestrate each module in the project step sequence.

A particularly effective framework for implementing a research compendium with these features is the Quarto website. On the one hand, Quarto documents, as literal programming is in general, provides rich support for integrating source content, computations, and visualizations in a single document. In addition, Quarto documents are designed to be modular --each is run in a separate R session making no assumptions about inputs or previous computing states. When tied to logical processing steps, this can help to ensure that each step says what it does, and does what it says, enhancing the transparency and reproducibility of the research project.

The Quarto website treats each document as part of a set of documents that are coordinated by the `_quarto.yml` configuration file. Rendering a Quarto website will execute and compile the Quarto documents as determined in the configuration settings. In this way, the goal of easy execution of the project is satified in a way that is consistent and predictable.

Creating the scaffolding for a research compendium in Quarto is a matter of creating a new Quarto website and adding the necessary files, directories, and documentation. The scaffold can be created through RStudio projects, using the R console with `quarto::quarto_create_project()`, or by using `quarto` from the command line.
...
@exm-quarto-sites, shows two Quarto website structures. On the left, appears the default structure for a Quarto website. It is minimal, and requires the addition of the necessary files. On the right, appears a more complete structure, with the addition of directories for inputs, outputs, and scripts, as well as a README file.
...
- Installs Ubuntu, updates apt-get
- Installs R
- Adds the research compendium to the container
- Installs `renv` and restores the project's R environment
-->
...
@lst-dockerfile-example, when executed with Docker in project directory, will create an image file. A Docker image is a blueprint for creating a container or any number of containers which contain the computing environment specified in the Dockerfile. In our example, the first layer adds the operating system, the second layer updates the system dependencies, the third layer installs R, and the fourth layer adds the research compendium to the container.

Notice that the operating system is `ubuntu:latest`. `ubuntu` refers to a version of the Ubuntu operating system, which is based on Linux. In line with our goal to use open source software, Ubuntu is a popular choice for Docker images. The `:latest` tag refers to the most recent version of Ubuntu. This can be set to a fixed version, such as `ubuntu:20.04`, to ensure that the container is built with a specific version of Ubuntu. Furthermore, the version of R that is installed can be set to a specific version.

Thanks to helpful R community members, there are also Docker images built specifically for the R community and distributed as part of the Rocker Project. These images include a variety of R versions and R environment setups (e.g., RStudio Server, Shiny Server, etc.). These images can be used as a base image in a Dockerfile, and can be found on Docker Hub.
...
@lst-dockerfile-rstudio is a much simpler and efficient Dockerfile. In addition, the `rstudio` image includes RStudio Server, which can be accessed through a web browser. This can be useful for sharing the research project with others, as it allows for the execution of the project in a web-based environment.

There are two more elements to address, however, the R packages in the `renv` lockfile and potential hardware differences across machines. The R packages can added to the image by adding the following lines to the Dockerfile.
...
The hardware considerations concern whether the container will run on the same architecture as the host system. The most widely used architecture is known as AMD64. This is the architecture used by most modern computers. Without any special considerations, Docker images, and thus their containers, will be built for the AMD64 architecture.

::: {.callout}
**{{< fa medal >}} Dive deeper**

The AMD64 architecture is used by the wide majority of computer manufacturers in their machines. Recently, however, ARM64 has been gaining popularity with the introduction of Apple's M1 chip. AMD64 and ARM64 are not compatible, so a container built for one architecture will not run on the other. To address this, Docker provides a way to build images to create containers for different architectures using `buildx`. For more information, see the [Docker documentation](https://docs.docker.com/desktop/multi-arch/).
:::

<!-- Publishing repositories -->

Once the research compendium and computing environment are prepared, the project can be published. There are many platforms for publishing research projects, including GitHub, Open Science Framework, and Zenodo. Each of these platforms has its own strengths and weaknesses, but they all provide a means to share research projects and collaborate with other researchers.
...
**What**: [Manage project and computing environments](https://qtalr.github.io/resources/recipes/recipe-11.html)\
