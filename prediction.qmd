...
We will use the `recipe()` function from the `recipes` package to create the recipe. The `recipe()` function minimally takes a formula and a data frame and returns a `recipe` object. The formula specifies the outcome variable ($y$) and the feature variable(s) ($x_1 .. x_n$). For example `y ~ x` can be read as "y as a function of x". In our particular case, we will use the formula `outcome ~ text` to specify that the outcome variable is the `outcome` variable and the feature variable is the `text` variable. The code is seen in @exm-pda-class-recipe.
...
**What**: [Predictive Data Analysis](https://github.com/qtalr/lab-09)\
