...
# Let's gauge the variablity of the placement_score variable by proficiency. A boxplot will be useful for this. Before we do that, let's make proficiency a factor variable in the order we want.
...
- leave the tokenfilter at a hardcoded number to start, start low (100) and then increase it to see how it affects the model's performance
...
- Note the `step_tokenize()` uses the `tokenizers` package function, which can customize the tokenization process. This is useful for languages with non-standard tokenization rules.
-->
...
The resulting engineered features data frame has `r nrow(cls_bake)` observations and `r ncol(cls_bake)` variables. The first variable is the outcome variable and the remaining variables are the engineered features. We can see that the recipe performed as expected and that the feature variables are the token counts for each word in the text.
...
Furthermore, the features are prefixed with the variable name and transformation step labels. In @exm-pda-class-recipe-prep we applied term frequency to the `text` variable. Therefore we see that our features are prefixed with `tf_text_`.
:::
...
In @fig-pda-class-tune-hyperparameters-evaluate-workflow-cv-roc, each fold is plotted. We can see that the AUC is quite large for most of the folds which suggests that our model is robust.
