...
\begin{document}
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, frame hidden, boxrule=0pt, colback={codebgcolor}, enhanced, breakable, borderline west={3pt}{0pt}{shadecolor}]}{\end{tcolorbox}}\fi
...
qtalrkit & 0.0.1.9400 & Github
(qtalr/qtalrkit@f644a93f0ae719b2155f418c679642bcef12696b) \\
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, arc=.35mm, breakable, leftrule=.75mm, bottomtitle=1mm, colback=white, bottomrule=.15mm, titlerule=0mm, opacityback=0, opacitybacktitle=0.6, toptitle=1mm, colframe=quarto-callout-tip-color-frame, colbacktitle=quarto-callout-tip-color!10!white, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Draft}, rightrule=.15mm, left=2mm, coltitle=black]
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, toprule=.15mm, arc=.35mm, breakable, leftrule=.75mm, left=2mm, colback=white, bottomrule=.15mm, rightrule=.15mm]
...
\item
  ...
  2022})
\item
  \texttt{usethis} (\protect\hyperlink{ref-R-usethis}{Wickham et al.
  2023})
\item
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, arc=.35mm, breakable, leftrule=.75mm, bottomtitle=1mm, colback=white, bottomrule=.15mm, titlerule=0mm, opacityback=0, opacitybacktitle=0.6, toptitle=1mm, colframe=quarto-callout-caution-color-frame, colbacktitle=quarto-callout-caution-color!10!white, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, rightrule=.15mm, left=2mm, coltitle=black]
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.7500}}@{}}
\caption{\label{tbl-ad-cedel2-do}Data origin file for the CEDEL2
corpus}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
attribute
...
diversity of APIs available. ROpenSci maintains a curated list of R
packages that provide access to data from web services. Visit the
...
\href{https://www.gutenberg.org/}{Project Gutenberg} API, the
\href{https://developer.twitter.com/en/docs}{Twitter} API, and the
\href{https://developers.facebook.com/docs/apis-and-sdks}{Facebook} API.
In this section, we will explore the
\href{https://talkbank.org/}{TalkBank} API, which provides access to a
large collection of spoken language corpora. We will use the
{[}TBDBr{]}(
...
\item[$\square$]
  ...
  \item
    ...
    \tightlist
    \item
      Useful for a study aiming to research spoken British English,
      either in isoloation or in comparison to American English (SWDA).
    \end{itemize}
  \item
    Introduce \texttt{TBDBr} package.

    \begin{itemize}
    \tightlist
    \item
      Show available functions in the package.
      \texttt{ls(getNamespace("TBDBr"))}
    \item
      Basic arguments for \texttt{get*()} functions: \texttt{corpusName}
      and \texttt{corpora}

      \begin{itemize}
      \tightlist
      \item
        Same for these function, demonstrate. So, ideally we could use
        one function to get all the data we need.
      \item[$\square$]
        Write custom function to download all resources for a TalkBank
        API resource.
      \end{itemize}
    \end{itemize}
  \end{itemize}
\item[$\square$]
  ...
  \tightlist
  \item[$\square$]
    In the relevant script, \texttt{1\_acquire\_data.qmd} in this case.
  \item[$\square$]
    In a separate file, \texttt{functions.R} in this case.

    \begin{itemize}
    \tightlist
    \item[$\square$]
      If you have multiple functions, it is good practice to put them in
      a separate file. Cleans up the main script and makes it easier to
      find the functions you need and reuse them in other scripts, if
      needed.
    \end{itemize}
  \end{itemize}
\end{itemize}
...
practice to source your functions early on a script so that it is
available for use throughout the script.

(note relative path to functions.R)

\begin{example}[]\protect\hypertarget{exm-ad-source-function}{}\label{exm-ad-source-function}
...
\FunctionTok{source}\NormalTok{(}\AttributeTok{file =} \StringTok{"functions.R"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Writing functions is a great way to make your code more reproducible. If
you find yourself repeating the same code over and over again, it is a
good candidate for a function. Functions can be used to automate
repetitive tasks, but they can also be used to make your code more
readable. If you find yourself writing code that is difficult to
understand, it is a good candidate for a function. Functions can be used
to make your code more readable by abstracting away the details of the
code and providing a name that describes what the code does. This is
especially useful when you are writing code that will be used by others.
...
\tightlist
\item[$\square$]
  Add example of \texttt{rvest::read\_html()}
\item[$\square$]
  ? Add example of \texttt{httr::GET()} with \texttt{httr::content()}.
\item[$\square$]
  \texttt{xml2::write\_html()}
\end{itemize}
...
\item[$\square$]
  ...
  \item[$\square$]
    ...
    \tightlist
    \item[$\square$]
      Examples: \texttt{gutenbergr}, \texttt{rtweet}, \texttt{rtoot},
      \texttt{rwhatsapp}, etc.
    \end{itemize}
  \end{itemize}
\item[$\square$]
  Introduce TalkBank repository, API, and \texttt{TBDBr} package.
...
{\centering \includegraphics[width=2.44in,height=\textheight]{figures/acquire-data/ad-example-webpage.png}
...
package, let's turn to a realistic example.

Say we want to acquire \ldots{} . The first step in any web scrape is to
investigate the site and page(s) we want to scrape to ascertain if there
any licensing restrictions. Many, but not all websites, will include a
plain text file
...
\CommentTok{\# paths\_allowed(paths = "https://www.last.fm/") \# check permissions}
\end{Highlighting}
\end{Shaded}
...
I have identified, seen in \textbf{?@fig-ad-example-lyrics-page-lastfm}.
...
From \textbf{?@fig-ad-inspect-element-artist-lastfm} we see that the
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, arc=.35mm, breakable, leftrule=.75mm, bottomtitle=1mm, colback=white, bottomrule=.15mm, titlerule=0mm, opacityback=0, opacitybacktitle=0.6, toptitle=1mm, colframe=quarto-callout-warning-color-frame, colbacktitle=quarto-callout-warning-color!10!white, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, rightrule=.15mm, left=2mm, coltitle=black]
...
\textbf{?@fig-ad-genre-tracks-list-lastfm}, we can see that a relative
...
data (\protect\hyperlink{ref-Adel2020}{Ädel 2020}). For a corpus sample
acquired from a repository will often include documentation which will
outline the sampling frame --this most likely will be the very
information which leads a researcher to select this resource for the
project at hand. It is important to include this documentation (HTML or
PDF file) or reference to the documentation (article citation or
link\footnote{Note that web links can change and it is often best to
  safeguard the documentation by downloading the HTML documentation page
  instead of linking}) within the reproducible project's directory
structure.
