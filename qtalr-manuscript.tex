...
\begin{tcolorbox}[enhanced jigsaw, colback=white, rightrule=.15mm, breakable, bottomrule=.15mm, colframe=quarto-callout-color-frame, toprule=.15mm, arc=.35mm, opacityback=0, leftrule=.75mm, left=2mm]
...
When a reference to a snippet of file or code, it will appear as in
Snippet 0.1.
...
the engines represent R packages that implement the model and therefore
you need to have the package installed to use the engine.
...
linear regression model. The RSME is 16.1 and the \(R^2\) is 0.5. And,
...
shows a glimpse of the \texttt{\_quarto.yml} configuration file for a
...
Let's look at a sample Dockerfile in
Snippet~\ref{lst-dockerfile-example}.
...
Snippet~\ref{lst-dockerfile-example}, when executed at the CLI with
...
Snippet~\ref{lst-dockerfile-rstudio} is a much simpler and efficient
...
image by adding the lines in Snippet~\ref{lst-dockerfile-renv} to the
Dockerfile.
...
Snippet~\ref{lst-quarto-repo-proj}.
