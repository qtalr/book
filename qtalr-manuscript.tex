...
\begin{tcolorbox}[enhanced jigsaw, breakable, colframe=quarto-callout-color-frame, leftrule=.75mm, bottomrule=.15mm, arc=.35mm, toprule=.15mm, left=2mm, opacityback=0, colback=white, rightrule=.15mm]
...
{\includegraphics{figures/p-diki.drawio.png}}
...
\tightlist
\item
  \{tidyverse\} (\citeproc{ref-R-tidyverse}{Wickham 2023b})
\item
  \{pak\} (\citeproc{ref-R-pak}{Csárdi and Hester 2023})
\item
  \{tinytex\} (\citeproc{ref-R-tinytex}{Xie 2023})
\item
  \{swirl\} (\citeproc{ref-R-swirl}{Kross et al. 2020})
\end{itemize}
...
\{reprex\} package (\citeproc{ref-R-reprex}{Bryan et al. 2024}), consult
the Resources Kit ``Creating reproducible examples'' guide.
...
listed above in ``\hyperref[sec-p-getting-started]{Getting started}'' of
this Preface and complete them before proceeding.
...
{\includegraphics[width=0.6\textwidth,height=\textheight]{part_1/figures/ta-ds-venn.drawio.png}}
...
(\citeproc{ref-Ide2001}{Ide and Macleod 2001}) may be better suited to
...
in their research. Consult {``US Copyright Office''}
...
(part-of-speech), \texttt{num\_let}, and \texttt{token\_id} all describe
the word.
...
(\citeproc{ref-DeMarneffe2021}{de Marneffe et al. 2021}), we can
generate linguistic annotation for each word in the \texttt{sentence}
variable.
...
\caption{Data origin
information}\label{tbl-ud-data-origin}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Information
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
information}\label{tbl-ud-data-dictionary}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Information
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subcaption{\label{fig-aa-belc-barplots-1}Bar plot}
...
\subcaption{\label{fig-aa-belc-barplots-2}Bar plot}
...
for example, Figure~\ref{fig-aa-belc-histograms-3}. We see that there is
a bin near the value 1.0. Given that the type-token ratio is a ratio of
the number of types to the number of tokens, it is unlikely that the
type-token ratio would be exactly 1.0 as this would mean that every word
in an essay is unique. Another, less dramatic, example is the bin to the
far right of Figure~\ref{fig-aa-belc-histograms-1}. In this case, the
bin represents the number of tokens in an essay. An uptick in the number
of essays with a large number of tokens is not surprising and would not
...
\caption{\label{fig-aa-belc-histogram-boxplot}Understanding the
...
\caption{\label{tbl-aa-belc-pivot-table}Pivot table for the mean
...
\texttt{tokens} in the BELC dataset. From the boxplot in
Figure~\ref{fig-aa-belc-boxplot-violin-plot-1}, we see that the general
trend towards more tokens used by students in higher groups. But we can
also appreciate the dispersion of the data within each group looking at
the boxes and whiskers. On the surface it appears that the data for
groups T1 and T3 are closer to each other than groups T2 and T4, in
which there is more variability within these groups. Furthermore, we can
see outliers in groups T1 and T3, but not in groups T2 and T4. From the
violin plot in Figure~\ref{fig-aa-belc-boxplot-violin-plot-2}, we can
see the same information, but we can also see the overall shape of the
...
and \texttt{types} in the BELC dataset. We see that there is an apparent
positive relationship between these two variables, which is consistent
with the idea that as the number of types increases, the type-token
ratio increases. In other words, as the number of unique words
increases, so does the lexical diversity of the text. Since we are
...
{\includegraphics{part_2/figures/a-ida-variables.drawio.png}}

}

\caption{\label{fig-aa-ida-variables}Roles of variables in inferential
data analysis}
...
Example~\ref{exm-ad-swda-unarchive-location} now shows the
\emph{swda.tar.gz} file and the \emph{swda} directory that contains the
unarchived directories and files.

\begin{example}[]\protect\hypertarget{exm-ad-swda-unarchive-location}{}\label{exm-ad-swda-unarchive-location}
...
same as in Example~\ref{exm-ad-swda-unarchive-location}, minus the
...
turn to \{fs\} (\citeproc{ref-R-fs}{Hester, Wickham, and Csárdi 2023}).
...
main page. However, \{TBDBr\} (\citeproc{ref-R-TBDBr}{Kowalski and
...
(\citeproc{ref-R-tidyr}{Wickham, Vaughan, and Girlich 2024}).
...
\textbf{?@tbl-td-europarl-normalization}.

;:: \{\#tbl-td-europarl-normalization tbl-colwidths=``{[}30, 70{]}''
.striped\}
...
Characteristics of the Europarl Corpus dataset that may require
normalization :::
...
investigation}\label{tbl-td-generation-idealized}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
variable
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\NormalTok{masc\_lemma\_disp\_thr }\SpecialCharTok{|\textgreater{}}
  ...
  \FunctionTok{pull}\NormalTok{(type) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{matrix}\NormalTok{(}\AttributeTok{ncol =} \DecValTok{5}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{kable}\NormalTok{(}\AttributeTok{col.names =} \ConstantTok{NULL}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
...
\textbar be \textbar think \textbar work \textbar also \textbar t
\textbar{}\\
\textbar have \textbar more \textbar year \textbar way \textbar first
\textbar{}\\
\textbar do \textbar just \textbar come \textbar need \textbar help
\textbar{}\\
\textbar not \textbar time \textbar use \textbar back \textbar day
\textbar{}\\
\textbar n't \textbar so \textbar well \textbar here \textbar many
\textbar{}\\
\textbar say \textbar other \textbar look \textbar new \textbar man
\textbar{}\\
\textbar go \textbar see \textbar then \textbar find \textbar ask
\textbar{}\\
\textbar know \textbar people \textbar right \textbar give \textbar very
\textbar{}\\
\textbar get \textbar take \textbar only \textbar thing \textbar much
\textbar{}\\
\textbar make \textbar now \textbar want \textbar tell \textbar even
\textbar{}\\
\bottomrule
\end{tabular}
...
~~~~~~~~~~\textbar{} \textbar{} \textbar{} \textbar{} \textbar{}\\
\textbar:----------\textbar:----------\textbar:----------\textbar:------------\textbar:------------\textbar{}\\
\textbar dump \textbar tuition \textbar blur \textbar wound
\textbar accidentally \textbar{}\\
\textbar instrument \textbar awaken \textbar litigation \textbar devote
\textbar protective \textbar{}\\
\textbar triumph \textbar hook \textbar resistance \textbar alright
\textbar preside \textbar{}\\
\textbar harsh \textbar fetch \textbar absurd \textbar evolutionary
\textbar rethink \textbar{}\\
\textbar ignorance \textbar brave \textbar qualify \textbar envy
\textbar strictly \textbar{}\\
\textbar sting \textbar unleash \textbar liberty \textbar nostalgic
\textbar wax \textbar{}\\
\textbar mainstream \textbar prosperous \textbar going \textbar shy
\textbar faith-based \textbar{}\\
\textbar wildly \textbar buzz \textbar awkward \textbar proximity
\textbar decidedly \textbar{}\\
\textbar dismiss \textbar presume \textbar afterwards \textbar sandy
\textbar resolute \textbar{}\\
\textbar liability \textbar summarize \textbar eve \textbar interfere
\textbar evidently \textbar{}\\
\bottomrule
\end{tabular}
...
\caption{\label{fig-eda-masc-pos-kmeans-elbow}Elbow method for
...
with the \texttt{tidyverse}, \texttt{infer} provides a set of functions
that can be used to specify the variables of interest, calculate the
observed statistic, generate the null hypothesis distribution and
calculate the \(p\)-value and the confidence interval.
...
\texttt{languageR} package. It contains over 3k observations describing
the realization of the recipient clause in English dative constructions.
To familiarize ourselves with the dataset, let's consider the data
...
our observed statistic, as seen in Example~\ref{exm-ida-cat-p-value}.
...
\subcaption{\label{fig-ida-cat-bivariate-length-ratio-log-1}RCP by
modality}
...
\subcaption{\label{fig-ida-cat-bivariate-length-ratio-log-2}RCP by
length ratio}
...
# A tibble: 3 x 3
  ...
  <chr>               <dbl>    <dbl>
1 intercept          -0.682   -0.455
2 length_ratio_log   -1.79    -1.49 
3 modalitywritten     0.802    1.24 
\end{verbatim}
...
rate of filler use between men and women, as the \(p\)-value is less
than 0.05.
...
The \(p\)-value in Example~\ref{exm-ida-num-bi-edu} was calculated using
a two-sided test, which is appropriate when the expected directionality
is not known. In this case, while we do have an expected directionality,
...
Conway, Drew. 2010. {``The Data Science Venn Diagram.''} \emph{The Data
...
Csárdi, Gábor, and Jim Hester. 2023. \emph{Pak: Another Approach to
Package Installation}. \url{https://pak.r-lib.org/}.
...
Daniel Zeman. 2021. {``Universal Dependencies.''} \emph{Computational
...
Du Bois, John W., Wallace L. Chafe, Charles Meyer, Sandra A. Thompson,
Robert Englebretson, and Nii Martey. 2005. {``Santa Barbara Corpus of
...
Github. 2024. {``GitHub.''} Repository. \emph{GitHub}.
https://github.com.
...
Ide, Nancy, and Catherine Macleod. 2001. {``The American National
Corpus: A Standardized Resource for American English.''} In
\emph{Proceedings of Corpus Linguistics}. Lancaster UK.
...
Macwhinney, Brian. 2024. {``TalkBank.''} Repository. \emph{The TalkBank
System}. https://talkbank.org/.
...
Microsoft. 2024. {``Visual Studio Code.''} Software. \emph{Visual Studio
Code - Code Editing. Redefined.} https://code.visualstudio.com/.
...
ROpenSci. 2024. {``The r-Universe System.''} Repository. \emph{The
R-Universe System}. https://ropensci.org/r-universe/.
...
Silveira, Natalia, Timothy Dozat, Marie-Catherine de Marneffe, Samuel
Bowman, Miriam Connor, John Bauer, and Christopher D. Manning. 2014.
{``A Gold Standard Dependency Corpus for English.''} In
\emph{Proceedings of the Ninth International Conference on Language
...
Software. \emph{R: The R Project for Statistical Computing}.
