...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[breakable, sharp corners, enhanced, colback={codebgcolor}, frame hidden, boxrule=0pt]}{\end{tcolorbox}}\fi
\makeatother
\makeatletter
\@ifpackageloaded{fontawesome5}{}{\usepackage{fontawesome5}}
\makeatother
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, colframe=quarto-callout-color-frame, leftrule=.75mm, bottomrule=.15mm, left=2mm, breakable, rightrule=.15mm, toprule=.15mm, arc=.35mm, opacityback=0]
...
operator, if the package is not loaded, \texttt{data(dative)} or
\texttt{languageR::dative}, respectively.
...
use \{DBI\} (\citeproc{ref-R-DBI}{R Special Interest Group on Databases
(R-SIG-DB), Wickham, \& Müller, 2024}) to connect to the database and
\{dbplyr\} (\citeproc{ref-R-dbplyr}{Wickham, Girlich, \& Ruiz, 2023}) to
query the database using the SQL language. Files from SPSS
(\emph{.sav}), SAS (\emph{.sas7bdat}), and Stata (\emph{.dta}) can be
read into R using \{haven\} (\citeproc{ref-R-haven}{Wickham, Miller, \&
Smith, 2023}).
...
(\citeproc{ref-R-stringr}{Wickham, 2023a}). This package provides a
...
ability to remove or replace text based on literal strings and Regular
Expressions.
...
calculate the cumulative frequency of the lemmas in the
\texttt{masc\_tbl} data frame, we can see that the top 10 types account
for around 25\% of the lemmas used in the entire corpus --by 100 types
that increases to near 50\% and 1,000 around 75\%, as seen in
...
(\citeproc{ref-Gries2023}{S. T. Gries, 2023, pp. 87--88}). The \(dp\)
...
as in Example~\ref{exm-explore-masc-metrics-modality}.
...
Figure~\ref{fig-explore-masc-verb-part-network}.
...
component with a scree plot. The \texttt{fviz\_eig()} function from
...
From Figure~\ref{fig-explore-masc-dtms-pca-scree}, we can see that the
first component shows the most variance explained, around 30\%, and then
drops for subsequent drops as the number of dimensions increase.
Visually we will apply the elbow method to identify the number of
dimensions to use for clustering. It appears hhe variance explained
decreases between 4 and 5 dimensions. This is a good indication that we
should use 4 dimensions for our clustering algorithm.
...
a data frame and returns a \texttt{recipe} object. The formula specifies
the outcome variable (\(y\)) and the predictor variable(s)
(\(x_1 .. x_n\)). For example \texttt{y\ \textasciitilde{}\ x} can be
read as ``y as a function of x''. In our particular case, we will use
the formula \texttt{outcome\ \textasciitilde{}\ text} to specify that
the outcome variable is the \texttt{outcome} variable and the predictor
variable is the \texttt{text} variable. The code is seen in
...
Example~\ref{exm-predict-class-recipe-prep} we applied term frequency -
inverse document frequency to the \texttt{text} variable. Therefore the
features are prefixed with \texttt{tfidf\_text\_}.
...
cross-validation. The \texttt{vfold\_cv()} function from \{rsample\}
takes a data frame and a number of folds and returns a
...
statistical designs and analyses. In Table~\ref{tbl-infer-cat-design},
we see common design scenarios, the variables involved, and the
...
by using the \texttt{fct\_relevel()} function from \{forcats\}. The
reason we would want to do this is to make the reference level more
interpretable. In our case, we would want to make the spoken modality
the reference level it allows us to estimate the difference of the
proportion of NP realizations of the recipient as a positive value.
Remember that in
...
# A tibble: 3 x 3
  ...
  <chr>               <dbl>    <dbl>
1 intercept          -0.682   -0.451
2 length_ratio_log   -1.78    -1.49 
3 modalitywritten     0.807    1.24 
\end{verbatim}
...
values. \{effectsize\} provides a function
...
\href{fig-alt=\%22Layers\%20and\%20components\%20of\%20a\%20computational\%20research\%20environment.\%22}{\includegraphics{part_5/figures/nested-project.drawio.png}}
...
Wickham, H. (2023a). \emph{Stringr: Simple, consistent wrappers for
common string operations}. Retrieved from
\url{https://stringr.tidyverse.org}

\bibitem[\citeproctext]{ref-R-tidyverse}
Wickham, H. (2023b). \emph{Tidyverse: Easily install and load the
