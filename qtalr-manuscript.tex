...
\begin{tcolorbox}[enhanced jigsaw, left=2mm, breakable, arc=.35mm, colback=white, opacityback=0, rightrule=.15mm, colframe=quarto-callout-color-frame, bottomrule=.15mm, leftrule=.75mm, toprule=.15mm]
...
predictive models for text analysis. We will discuss the research goals
that are typically addressed using supervised learning and then
estabnlish a workflow for building predictive models, including the
steps for preparing data, training and testing models, and evaluating
and reporting results.

\subsection{Goals and approach}\label{sec-pda-goals-approach}
...
\texttt{text} variable. Therefore we see that our features are prefixed
with \texttt{tfidf\_text\_}.
...
>   <fct>          <dbl>            <dbl>         <dbl>             <dbl>
> 1 Learner       0                     0             0                 0
> 2 Learner       0.0248                0             0                 0
> 3 Learner       0.0382                0             0                 0
> 4 Learner       0                     0             0                 0
> 5 Learner       0.0687                0             0                 0
\end{verbatim}
...
We will use the \texttt{predict()} function to predict the outcome on
the test set. The \texttt{metrics()} function takes a data frame with
...
Now a model that can predict the proficiency level of a writer based on
their writing sample is useful, as a task. You could imagine that this
could be a preprocessing step for a language learning application, for
example. The model could be used to filter out writing samples that are
not at the right proficiency level for the user. This is a simple
example, but the model could be used in many different ways.

But for a study that is more interested in learning about what features
are most important for predicting the proficiency level of a writer, we
still have some work to do. We can inspect the errors on the test set to
gain some insight into what what writing samples, and the proficiency
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Collect predictions}
...
is also a method which can be better suited for high-dimensionalensional
data where many values are zero, that is, sparse data, which is often
the case with text data.
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, breakable, toptitle=1mm, opacityback=0, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, leftrule=.75mm, bottomtitle=1mm, left=2mm, opacitybacktitle=0.6, coltitle=black, colback=white, colbacktitle=quarto-callout-caution-color!10!white, arc=.35mm, colframe=quarto-callout-caution-color-frame, rightrule=.15mm, toprule=.15mm, titlerule=0mm]
