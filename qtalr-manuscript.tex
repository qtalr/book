...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, breakable, colframe=quarto-callout-color-frame, rightrule=.15mm, opacityback=0, bottomrule=.15mm, colback=white, toprule=.15mm, arc=.35mm, left=2mm]
...
\item
  \texttt{swirl} (\citeproc{ref-R-swirl}{\textbf{R-swirl?}})
\end{itemize}
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-00.html}{Literate
...
\includegraphics[width=4.04in,height=\textheight]{part_1/figures/ta-data-collection-methods.drawio.png}
...
\includegraphics[width=0.75\textwidth,height=\textheight]{part_1/../figures/p-diki.drawio.png}
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-01.html}{Academic
writing with Quarto}\\
...
\includegraphics[width=2.76in,height=\textheight]{part_2/figures/ud-tidy.drawio.png}
...
\includegraphics[width=2.65in,height=\textheight]{part_2/figures/ud-transformations.drawio.png}
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-02.html}{Reading,
...
\includegraphics[width=0.75\textwidth,height=\textheight]{part_2/figures/ud-diki.drawio.png}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{part_2/figures/a-word-mapper.png}
...
\includegraphics{part_2/3_analysis_files/figure-pdf/fig-aa-belc-boxplot-trimmed-1.pdf}
...
\includegraphics{part_2/3_analysis_files/figure-pdf/fig-aa-belc-boxplot-trimmed-2.pdf}
...
\includegraphics{part_2/3_analysis_files/figure-pdf/fig-aa-belc-histogram-density-trimmed-1.pdf}
...
\includegraphics{part_2/3_analysis_files/figure-pdf/fig-aa-belc-histogram-density-trimmed-2.pdf}
...
\includegraphics{part_2/3_analysis_files/figure-pdf/fig-aa-belc-histogram-density-trimmed-3.pdf}
...
\includegraphics[width=0.75\textwidth,height=\textheight]{part_2/figures/a-eda-variables.drawio.png}
...
\includegraphics[width=0.75\textwidth,height=\textheight]{part_2/figures/a-pda-variables.drawio.png}
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-03.html}{Descriptive
...
\includegraphics[width=0.75\textwidth,height=\textheight]{part_2/figures/a-diki.drawio.png}
...
(\citeproc{ref-R-workflowr}{\textbf{R-workflowr?}}),
\href{http://projecttemplate.net/}{ProjectTemplate}
(\citeproc{ref-R-ProjectTemplate}{\textbf{R-ProjectTemplate?}}), and
\href{https://github.com/ropensci/targets}{targets}
(\citeproc{ref-R-targets}{\textbf{R-targets?}}), but there are many
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-04.html}{Understanding
...
\includegraphics[width=0.75\textwidth,height=\textheight]{part_2/figures/r-diki.drawio.png}
...
(\citeproc{ref-R-wordbankr}{\textbf{R-wordbankr?}}), and
...
(\citeproc{ref-R-lingtypology}{\textbf{R-lingtypology?}}).
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-05.html}{Collecting
...
\texttt{readtext} package
(\citeproc{ref-R-readtext}{\textbf{R-readtext?}}), we can read the text
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-06.html}{Organizing
...
\texttt{jiebaR} package (\citeproc{ref-R-jiebaR}{\textbf{R-jiebaR?}})
and the \texttt{gibasa} package
(\citeproc{ref-R-gibasa}{\textbf{R-gibasa?}}).
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-07.html}{Transforming
and documenting datasets}\\
...
for spot identifying collocations of a target word or phrase, however it
can be time-consuming and is not suitable for large datasets.
...
documents (\citeproc{ref-Petrenz2011}{Petrenz and Webber 2011}).
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-08.html}{Exploratory
analysis methods}\\
...
\textbf{overfitting} and by holding out a portion of the data for
testing, we can evaluate the model's performance on data that it has not
seen before and therefore get a more accurate estimate of the
generalizable trends in the data.
...
(\citeproc{ref-Baayen2011}{2011}) for more information.
...
most of our cells will be filled with zeros. This is a sparse matrix.
...
As you can appreciate the model building process is iterative and many
of the steps are the same. This is a good indication that creating a
custom function to build and tune the model would be a good idea.
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-09.html}{Building
predictive models}\\
...
For the 1,000 samples, the proportion statistic ranges from 0.48 to
...
hypothesis were true. This is called the p-value, and calculating this
estimate is step 4 in the workflow. The \textbf{p-value} is calculated
by counting the number of samples in the null hypothesis distribution
that are more extreme than expected within some level of uncertainty.
95\% is the most common level of uncertainty, which is called the alpha
level. The remain 5\% of the distribution is the space where the
likelihood that the null hypothesis accounts for the statistic is below
This means that if the p-value is less than 0.05, then we reject the
null hypothesis. If the p-value is greater than 0.05, then we fail to
...
In Table~\ref{tbl-ida-cat-bivariate}, both the proportions and raw
counts adorn the table. We can appreciate that the proportion of NP
...
>   <chr>               <dbl>    <dbl>
> 1 intercept          -0.673   -0.453
> 2 length_ratio_log   -1.79    -1.50 
> 3 modalitywritten     0.800    1.21
\end{verbatim}
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-10.html}{Building
inference models}\\
...
\begin{tcolorbox}[enhanced jigsaw, breakable, opacitybacktitle=0.6, colframe=quarto-callout-caution-color-frame, rightrule=.15mm, opacityback=0, colbacktitle=quarto-callout-caution-color!10!white, bottomrule=.15mm, colback=white, toprule=.15mm, coltitle=black, bottomtitle=1mm, toptitle=1mm, titlerule=0mm, left=2mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, arc=.35mm, leftrule=.75mm]
...
Table~\ref{tbl-ao-common-components} as it is a distillation of the
...
maintaining connection with `why this matters' is important.
...
\includegraphics[width=0.35\textwidth,height=\textheight]{part_5/figures/nested-project.drawio.png}

}

\caption{\label{fig-contr-ro-structure}Layers of a computational
...
Furthermore, hardware resources can vary widely. One of the most
important aspects of hardware to consider for reproducibility is the
architecture of the processor (the CPU).

For a quick overview of some of the elements of these layers, we can use
\texttt{session\_info()} from the \texttt{sessioninfo} package
(\citeproc{ref-R-sessioninfo}{Wickham et al. 2021}), as seen in
Example~\ref{exm-contr-ro-session-info}.

\begin{example}[]\protect\hypertarget{exm-contr-ro-session-info}{}\label{exm-contr-ro-session-info}

~

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Get the session information}
\NormalTok{session }\OtherTok{\textless{}{-}}\NormalTok{ sessioninfo}\SpecialCharTok{::}\FunctionTok{session\_info}\NormalTok{()}

\CommentTok{\# Print the platform information}
\NormalTok{session}\SpecialCharTok{$}\NormalTok{platform}
\end{Highlighting}
\end{Shaded}
...
From the output of Example~\ref{exm-contr-ro-session-info}, we can see
that the platform information for the current session includes the R
version 4.3.3 (2024-02-29), the operating system (macOS Ventura 13.6.3),
and the system (x86\_64, darwin22.6.0).
...
the processing steps. But it will also consist of a coordinating script,
that is used to orchestrate each module in the project step sequence.

A particularly effective framework for implementing a research
compendium with these features is the Quarto website. On the one hand,
Quarto documents, as literal programming is in general, provides rich
support for integrating source content, computations, and visualizations
in a single document. In addition, Quarto documents are designed to be
modular --each is run in a separate R session making no assumptions
about inputs or previous computing states. When tied to logical
processing steps, this can help to ensure that each step says what it
does, and does what it says, enhancing the transparency and
reproducibility of the research project.

The Quarto website treats each document as part of a set of documents
that are coordinated by the \texttt{\_quarto.yml} configuration file.
...
of creating a new Quarto website and adding the necessary files,
directories, and documentation. The scaffold can be created through
RStudio projects, using the R console with
\texttt{quarto::quarto\_create\_project()}, or by using \texttt{quarto}
...
\ExtensionTok{project/}
  ...
  \ExtensionTok{├──}\NormalTok{ about.qmd}
  \ExtensionTok{├──}\NormalTok{ index.qmd}
  \ExtensionTok{└──}\NormalTok{ styles.css}
\end{Highlighting}
\end{Shaded}
...
Example~\ref{exm-quarto-sites}, shows two Quarto website structures. On
the left, appears the default structure for a Quarto website. It is
minimal, and requires the addition of the necessary files. On the right,
appears a more complete structure, with the addition of directories for
inputs, outputs, and scripts, as well as a README file.
...
In File~\ref{lst-quarto-yml}, we see the \texttt{\_quarto.yml}
configuration file for a Quarto project website, as in
Example~\ref{exm-quarto-sites}. Rendering this project will produce a
website which includes the \texttt{index.qmd} document as the main page,
the \texttt{code/} directory as a navigation section with the rendered
QMD documents in the directory as subpages. The \texttt{output/} also
appears in the navigation, but as ``Reports''. While the project as a
whole will be rendered to HTML, individual documents can be rendered to
other formats. This can be leveraged to create PDF versions of
write-ups, for example, or use \texttt{revealjs} to create presentations
that are rendered and easily shared on the web.
...
project. This is where the \texttt{renv} package comes in.

\texttt{renv} is a package that helps manage R package installation by
versions. It does this by creating a separate environment for each R
project where \texttt{renv} is initialized. This environment allows us
to keep snapshots of the state of the project's R environment in a
lockfile --a file that contains the list of packages used in the project
and their versions. This can be helpful for developing a project in a
consistent environment and controlling what packages and package
versions you use and update. More importantly, however, if the lockfile
is shared with the project, it can be used to restore the project's R
environment to the state it was in when the lockfile was created, yet on
a different machine or at a different time.
...
however. These change will inevitably affect our ability to execute the
project (even if we are on the same machine, sometimes), but it is
surely more pronounced when we expect our project to run on a different
machine! To address these elements of the computing environment, we need
another, more computing-comprehensive approach.

A powerful and popular approach to reproducible software, operating
system, and hardware environments is to use Docker. Docker is software
that provides a way to create and manage entire computing environments.
These environments are called containers, and they are portable,
consistent, and almost entiely isolated from the host system. This means
that a container can be run on any machine that has Docker installed,
and it will run the same way regardless of the host system. The
technology behind Docker is called containerization, and is a type of
virtualization. Containers are widely used as they are quick to develop,
...
instructions for creating our reproducible computing environment. Aptly
named, a Dockerfile is a sequence of layers. Each layer pertains to a
different aspect of the computing environment, such as the operating
system, system dependencies, and software as well as any other resources
that are necessary for the project --including the research compendium.
...
# Add the research compendium to the container
COPY . /project
\end{verbatim}
...
File~\ref{lst-dockerfile-example}, when executed with Docker in project
directory, will create an image file. A Docker image is a blueprint for
creating a container or any number of containers which contain the
computing environment specified in the Dockerfile. In our example, the
first layer adds the operating system, the second layer updates the
system dependencies, the third layer installs R, and the fourth layer
adds the research compendium to the container.
...
that the container is built with a specific version of Ubuntu.
Furthermore, the version of R that is installed can be set to a specific
version.
...
environment setups (e.g., RStudio Server, Shiny Server, etc.). These
images can be used as a base image in a Dockerfile, and can be found on
Docker Hub.
...
execution of the project in a web-based environment.

There are two more elements to address, however, the R packages in the
\texttt{renv} lockfile and potential hardware differences across
machines. The R packages can added to the image by adding the following
...
architecture is known as AMD64. This is the architecture used by most
modern computers. Without any special considerations, Docker images, and
thus their containers, will be built for the AMD64 architecture.

\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, breakable, colframe=quarto-callout-color-frame, rightrule=.15mm, opacityback=0, bottomrule=.15mm, colback=white, toprule=.15mm, arc=.35mm, left=2mm]

\textbf{\faIcon{medal} Dive deeper}

The AMD64 architecture is used by the wide majority of computer
manufacturers in their machines. Recently, however, ARM64 has been
gaining popularity with the introduction of Apple's M1 chip. AMD64 and
ARM64 are not compatible, so a container built for one architecture will
not run on the other. To address this, Docker provides a way to build
images to create containers for different architectures using
\texttt{buildx}. For more information, see the
\href{https://docs.docker.com/desktop/multi-arch/}{Docker
documentation}.

\end{tcolorbox}

Once the research compendium and computing environment are prepared, the
project can be published. There are many platforms for publishing
research projects, including GitHub, Open Science Framework, and Zenodo.
Each of these platforms has its own strengths and weaknesses, but they
all provide a means to share research projects and collaborate with
other researchers.

\begin{itemize}
\item
  Image to Docker Hub or GitHub Container Registry
\item
  Push to GitHub or Open Science Framework
\item
...
\item
  ...
  \item
    ...
    \tightlist
    \item
      \texttt{.gitignore} for sensitive data
    \end{itemize}
  \end{itemize}
\item
...
\textbf{What}:
\href{https://qtalr.github.io/resources/recipes/recipe-11.html}{Manage
