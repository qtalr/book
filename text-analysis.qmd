...
- [x] Insert a section on Reproducible Research in this chapter and underscore how important it is to be able to reproduce your results: for yourself, for others, and for the scientific community. 
...
**What**: [Variables and vectors, Workspace](https://github.com/qtalr/swirl)\
...
knitr::include_graphics("figures/text-analysis/ta-ds-venn.drawio.png")
```
...
knitr::include_graphics("figures/text-analysis/ta-lang-venn.drawio.png")
```
...
knitr::include_graphics("figures/text-analysis/ta-data-collection-methods.drawio.png")
```
...
- [ ] Revise this intro to 'Text Analysis' section.
...
For research aimed at predicting some target outcome variable, **Predictive Data Analysis** (PDA) is employed. PDA is a supervised machine learning task that uses a set of features to predict a target outcome variable. The extent to which the the features can predict the outcome variable on new data is the primary evaluation metric. However, human interpretation is also important for many text analysis tasks to contextualize the results and to evaluate the extent to which the results are meaningful.

For research aimed at explaining relationships between variables and the population from which the sample was drawn, **Inferential Data Analysis** (IDA) is employed. IDA is theory-driven and uses statistical models to test hypotheses. The extent to which the results can be confidently generalized to the population is the primary evaluation metric. 
...
- morphology?
- syntax? 
  - Wasow paper @Wasow2005 (I used this in the Corpus Studies of Syntax, chapter)
  - Manning paper @Manning2003
:::
...
- Means to an end: text analysis as a tool for other language research methods (hypothesis generation, external valid data collection, linguistic annotation, methodological triangulation, etc. [@Francom2022])
...
**What**: [Literate programming II](https://qtalr.github.io/qtalrkit/articles/recipe-.html)\
