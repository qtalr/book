...
- [ ] Add a description of the importance of the unit of observation in the transformation process.
    - Note: that some units of observation will require that the data have more textual context (e.g. syntactic parsing) than others (e.g. word frequencies). It is important that the presentation of the transformation steps are no necessarily in the order in which they are applied.
-->
...
In this chapter, we will focus on transforming a curated dataset to refine and possibly expand its relational characteristics to align with our research. The transformation process is divided into four categories: normalization, tokenization, recoding, generation, and merging. These categories are not sequential but may occur in any order based on the researcher's evaluation of the dataset characteristics and the desired outcome.
...
**What**: [Reshaping datasets](https://github.com/qtalr/lessons)\
...
- Swirl lesson
  - Reshaping datasets (row-wise?)
  - Reshaping datasets (column-wise?)
...
- Generation (row-wise/ column-wise)
  - Lexical annotation
