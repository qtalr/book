...
::: {#first}
```{r}
#| label: tbl-td-merging-join-lexicon-contractions-list
#| tbl-cap: "Common contractions and expanded forms."
#| echo: false

it_is <-
  lexicon::key_contractions |>
  filter(contraction == "it's")
...
: Example tokenized dataset with contractions. {#tbl-td-merging-join-lexicon-contractions .sm .striped}
...
So they are not the same length. Using the `nrow()` function, I can see that the annotated dataset has `r nrow(enntt_natives_ann)` observations and the calculated dataset has `r nrow(enntt_natives_syn_comp)` observations. The annotation data frames will have many more observations due to the fact that the unit of observations is word tokens. The calculated data frames' unit of observation is the sentence.
...
**What**: [Dataset manipulation: tokenization and joining datasets](https://github.com/qtalr/lab-7)\
