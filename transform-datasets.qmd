...
With a (monster) regular expression that matches the filled pauses we are interested in, and only those filled pauses, we can move forward with counting the number of matches for each utterance. 

To do this we will use the `str_count()` function from the `stringr` package. This function counts the number of matches for a pattern in a character vector. We will use `mutate()` to create new variabled `uh` and `um` which will contain the counts for the filled pauses `uh` and `um`, respectively. The code is seen in @exm-tr-swda-recoding-filled-pauses.
