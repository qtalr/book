...
<!-- A population is a group of people, objects, or events that share a common characteristic or belong to a specific category. For example, the population of a city includes all the people who live within its geographical boundaries. It's important to note that populations are not fixed or objective entities, but rather idealized sets of objects and events that are subject to change over time. -->

Likewise when we talk about populations in terms of language we dealing with an objective and idealized aspect of reality. Let's take the words of the English language as an analog to our previous example population. In this case the words are the people and English is the bounding characteristic. Just as people, words move out, move in, are born, and pass away. Any compendium of the words of English at any moment is almost instananeously incomplete. This is true for all populations, save those relatively rare cases in which the bounding characteristics select a narrow slice of reality which is objectively measurable and whose membership is fixed (the complete works of Shakespeare, for example).
...
<!-- [ ]  Companion website link, instead of a list here -->
...
Each column in a tidy dataset is a **variable** which reflects measures for a particular attribute that can take on different values. These values are grouped into two main categories: **categorical** and **numeric**. 

<!-- Provide a brief description of the two types of variables and their sub-types. -->

Qualitative variables are **categorical** and can be further broken down into **nominal** and **ordinal**. Quantitative variables are **Numeric** and can be further broken down into **interval** and **ratio** as seen in @fig-ud-informational-values.
...
<!-- 
- [ ] This is a good moment to introduce the concept of the primary unit of analysis and the primary unit of observation.
